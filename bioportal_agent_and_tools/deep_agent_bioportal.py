...
def get_agent_bioportal(trusted_ontologies: list[str], term_ontologies: list[str]):
    ...
    Before producing JSON, you MUST call the BioPortal search tool at least once (e.g. find_term_in_ontology or find_best_definition).
    ...
    After you have selected the best BioPortal match, you MUST call classify_skos_match(...) to obtain the SKOS mapping type and its explanation.
    Return ONLY the JSON defined by Bioportalmapping (fields qid, skos, explanation). Do not use agentmapping_format or any additional wrapping.
